...
function AcceptInvitePage() {
  ...
  const { data: invitation, isLoading: isFetching } = useQuery(invitationQueries.getById(id))
    ...
    <div className="flex h-screen w-full items-center justify-center bg-muted/30">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            {invitation 
              ? `You have been invited by ${invitation.inviter?.name} to join their ${invitation.project_id ? "project" : "team"}.`
              : "You have been invited to join an organization. Click below to accept the invitation and gain access."}
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col items-center justify-center gap-4 py-6">
          {(status === "idle" || isFetching) && (
            <div className="flex size-16 items-center justify-center rounded-full bg-primary/10 text-primary">
              {isFetching ? <Loader2 className="size-8 animate-spin" /> : <CheckCircle2 className="size-8" />}
            </div>
          )}
          {status === "loading" && (
